...
router.post('/', async (req: Request, res: Response) => {
  ...
  try {
    ...
    const result = await pool.query(
      'INSERT INTO templates (user_id, name, category, template_text, is_pinned) VALUES ($1, $2, $3, $4, $5) RETURNING id',
...
router.put('/:id', async (req: Request, res: Response) => {
  ...
  try {
    ...
    const pinned = is_pinned ?? false;
    await pool.query(
      'UPDATE templates SET name = $1, category = $2, template_text = $3, is_pinned = $4 WHERE id = $5 AND user_id = $6',
...
router.delete('/:id', async (req: Request, res: Response) => {
  ...
  try {
    ...
    await pool.query('DELETE FROM templates WHERE id = $1 AND user_id = $2', [templateId, userId]);
