 ...
 * - Errors are thrown with descriptive messages to be caught by route handlers.
 * - Functions expect internal user IDs (from `getUserIdFromClerk`), not Clerk IDs.
 */

import pool from '../config/db';
import { getActionId } from '../utils/db';

// Interface for a chain step as received from the client
interface ChainStep {
  action: string;
  data: any; // JSONB in the database; type safety to be enhanced in Step 3
  step_order: number;
}

// Interface for a chain with steps as returned to the client
interface Chain {
  id: number;
  created_by: number;
  name: string;
 ...
 * @returns {Promise<Chain[]>} Array of chains with embedded steps.
 * @throws {Error} If a database error occurs.
 */
export async function getChains(userId: number): Promise<Chain[]> {
  const chainsResult = await pool.query(
    'SELECT * FROM prompt_chains WHERE created_by = $1',
    [userId]
  );
  const chains = chainsResult.rows as Chain[];
 ...
 * @param {number} userId - The internal user ID.
 * @returns {Promise<Chain>} The chain with embedded steps.
 * @throws {Error} 'Chain not found' if the chain doesn’t exist or isn’t owned by the user.
 * @throws {Error} If a database error occurs.
 */
export async function getChain(chainId: number, userId: number): Promise<Chain> {
  ...
  const chain = chainResult.rows[0] as Chain;
...
/**
 * Creates a new chain with its steps for a user.
 * 
 * @param {number} userId - The internal user ID.
 * @param {string} name - The name of the chain.
 * @param {ChainStep[]} steps - Array of steps to create.
 ...
 * @throws {Error} If a database error occurs.
 */
export async function createChain(userId: number, name: string, steps: ChainStep[]): Promise<number> {
...
/**
 * Updates an existing chain’s name and steps, if it belongs to the user.
 * 
 * @param {number} chainId - The ID of the chain.
 * @param {number} userId - The internal user ID.
 * @param {string} name - The updated name.
 * @param {ChainStep[]} steps - Array of updated steps.
 * @throws {Error} 'Chain not found' if the chain doesn’t exist or isn’t owned by the user.
 * @throws {Error} 'Invalid step data' if any step is missing required fields.
 * @throws {Error} 'Action not found: <name>' if an action is invalid.
 * @throws {Error} If a database error occurs.
 */
export async function updateChain(chainId: number, userId: number, name: string, steps: ChainStep[]): Promise<void> {
  if (!name || !steps || !Array.isArray(steps)) {
    throw new Error('Name and steps array are required');
  }
